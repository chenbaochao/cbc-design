...
$(document).on('click', 'button.friends_comment', function () {
    ...
    if (title == 1) {
        ...
        var all = "";
        $.ajax({
            url: '/api/critic/get/comment/'+pid,
            type: 'put',
            dataType: 'JSON',
            data: {pae: 1},
            success: function (resp) {
                ...
                for (var i = 0; i < data.length; i++) {
                    all_friends_comment += "<!--评论盖楼-->" +
                        ...
                        "<div>" +
                        "<img class='all_friends_user_img' src='" + data[i].avatar + "' title='" + data[i].nickname + "' value='" + data[i].avatar + "'/>" +
                        "<div style='padding-left: 36px;font-size: .8em'>" +
                        "<span class='all_friends_username' style='' value='" + data[i].userId + "'>" + data[i].nickname + "：</span>" +
